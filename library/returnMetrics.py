...
def returnMetric(patchesDeg, patchesRef, processingPath="processing/", height=192, width=192):
    ...
    patchesRefNP_YUV = np.repeat(patchesRefNP_YUV, repeats=2, axis=0)
    ...
    patchesDegNP_YUV = np.repeat(patchesDegNP_YUV, repeats=2, axis=0)
    ...
    runTerminalCmd(f"vmaf -r {videoFilePathRef} -d {videoFilePathDeg} -w {width} -h {height} -p 420 -b 8 --csv -o {outputCSVPath} --feature cambi")
    print(f"vmaf -r {videoFilePathRef} -d {videoFilePathDeg} -w {width} -h {height} -p 420 -b 8 --csv -o {outputCSVPath} --feature cambi")
    actualScores = pd.read_csv(outputCSVPath)['cambi'][::2]
